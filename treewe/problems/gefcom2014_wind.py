...
df_scores = pd.read_csv(os.path.join(parent_dir, 'data', 'gefcom2014', 'gefcom2014-wind-scores.csv'),
                        index_col=0)

windfarms = []
for name in df_data.columns.levels[0]:
    windfarms.append(ef.WindFarm(name=name,
                                capacity=1))

portfolio = ef.Portfolio(name="Portfolio", assets=windfarms)

dataset = ef.Dataset(name="gefcom2024-wind",
...
problem = Problem(name="gefcom2014-wind", 
                  environment=env,
                  objective=obj)